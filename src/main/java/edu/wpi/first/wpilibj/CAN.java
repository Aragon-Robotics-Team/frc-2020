...
/**
 * High level class for interfacing with CAN devices conforming to the standard
 * CAN spec.
 *
 * <p>
 * No packets that can be sent gets blocked by the RoboRIO, so all methods work
 * identically in all robot modes.
 *
 * <p>
 * All methods are thread safe, however the CANData object passed into the read
 * methods and the byte[] passed into the write methods need to not be modified
...
public class CAN implements Closeable {
    ...
    /**
     * Create a new CAN communication interface with the specific device ID. This
     * uses the team manufacturer and device types. The device ID is 6 bits (0-63).
     *
     * @param deviceId The device id
     */
    public CAN(int deviceId) {
        m_handle = CANAPIJNI.initializeCAN(kTeamManufacturer, deviceId, kTeamDeviceType);
        HAL.report(1, deviceId + 1);
    }

    /**
     * Create a new CAN communication interface with a specific device ID,
     * manufacturer and device type. The device ID is 6 bits, the manufacturer is 8
     * bits, and the device type is 5 bits.
     *
     * @param deviceId           The device ID
    ...
    /**
     * Read a CAN packet. The will return the last packet received until the packet
     * is older then the requested timeout. Then it will return false.
     *
     * @param apiId     The API ID to read.
