...
public class AnalogGyro extends GyroBase implements Gyro, PIDSource, Sendable, AutoCloseable {
    ...
    public void initGyro() {
        ...
        HAL.report(1, m_analog.getChannel() + 1);
     ...
     *
     * @param channel The analog channel the gyro is connected to. Gyros can only be
     ...
     * parameters for presetting the center and offset values. Bypasses calibration.
     *
     * @param channel The analog channel the gyro is connected to. Gyros can only be
     *                used on on-board channels 0-1.
     * @param center  Preset uncalibrated value to use as the accumulator center
    ...
    public AnalogGyro(AnalogInput channel, int center, double offset) {
        ...
        m_analog = channel;
        initGyro();
        AnalogGyroJNI.setAnalogGyroParameters(m_gyroHandle, kDefaultVoltsPerDegreePerSecond, offset, center);
        reset();
    }
